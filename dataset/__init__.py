...
def create_dataset_attn_maps(config):
    train_files = {}
    for train_name in config.train_tag.split('#'):
        if train_name not in config.train_file_dict:
            raise NotImplementedError
        train_files[train_name] = config.train_file_dict[train_name]
...
def create_dataset(config):
    ...
    for k, v in val_files.items():
        ...
        if type(v[0]) != list:
            v = [v]
        if config.use_external_attn_maps:
            attn_maps_file = f"{config.val_attn_maps_path}/infer_attn_maps_val_{k}.pt"
        else:
            attn_maps_file = None
        for val_file in v:
            datasets.append(ValDataset(
