# nohup bash scripts/batch_eval_fast3d_pred_attn.sh > "outputs/batch_eval_fast3d_pred_attn_run1.log" 2>&1 &
...
for ((i=0; i<len; i++)); do
    ...
    other_info="fast3d_pred_attn_${use_external_attn_maps}_layer${k}_rank${rank}_oracle${use_fast_v_oracle}_a_map_ori${use_a_map_ori}"
    ...
    CUDA_VISIBLE_DEVICES=0,1,2,3 torchrun --nproc_per_node=${gpu_num} \
        tasks/inference_fast3d_pred_attn.py \
