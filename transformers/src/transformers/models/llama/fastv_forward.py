    ...
    def forward(
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                USE_FAST_V = self.use_fast_v  # use fastv or not, True
                SYS_LENGTH = self.fast_v_sys_length  # the length of system prompt, the number of tokens before image tokens, llava is 36
                IMAGE_TOKEN_LENGTH = (
                    self.fast_v_image_token_length
                )  # the number of image tokens, llava is 576
                ATTENTION_RANK = self.fast_v_attention_rank  # the number of image tokens after pruning: (1-R)*576. If R=50%, this number will be 288.
                AGG_LAYER = self.fast_v_agg_layer  # the value of K: the layer from which to start pruning tokens. K=2 by default.
                ...
                if USE_FAST_V:
                    if idx < AGG_LAYER:
                        ...
                        if idx != 0:
                            ...
                            last_layer_attention_avg_last_tok = last_layer_attention_avg[
                                -1
                            ]
