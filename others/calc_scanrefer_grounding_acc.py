...
output_file = "/mnt/petrelfs/my_username/share/Chat-3D-v2/outputs/20240419_171033_dp0.1_lr5e-6_sta2_ep3_scanrefer_seg#scan2cap_seg#nr3d_caption_seg#obj_align_seg#scanqa_seg#sqa3d_seg__scanqa#scanrefer#sqa3d#scan2cap__debug/preds_epoch-1_step0_scanrefer.json"
...
if not os.path.exists(unique_multiple_lookup_file):
    ...
    all_sem_labels = {
        scene_id: np.array(all_sem_labels[scene_id]) for scene_id in all_sem_labels.keys()
    }
...
for i in range(len(split_nums) - 1):
    ...
    print(
        f"{split_nums[i]} <= x < {split_nums[i + 1]}: {tot} {iou25 / tot} {iou50 / tot}"
    )
