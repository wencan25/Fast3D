...
def collect_Nr3d_gt(now_scene_id):
    nr3d_gt_path = "/mnt/petrelfs/share_data/username_a/share/mask3d/M3DRef-CLIP/dataset/nr3d/metadata/nr3d_test.csv"
    # "/mnt/petrelfs/share_data/username_a/share/mask3d/mask3d_data/langdata/referit3d/nr3d.csv"
...
if __name__ == "__main__":
    ...
    scan_data = np.load(
        f"/mnt/hwfile/OpenRobotLab/my_username/openscene/data/scans/{scene_id}/pc_infos.npy"
    )
    mask3d_data = torch.load(
        f"/mnt/hwfile/OpenRobotLab/my_username/openscene/data/mask3d_ins_data/pcd_all/{scene_id}.pth"
    )

    gt_coords, gt_vertices, all_colors = (
        scan_data[:, :3],
        scan_data[:, 3:6],
        scan_data[:, 6:9],
    )
    ...
    if vis_bbox:
        if test_dataset == "scanrefer":
            scanrefer_label = json.load(
                open(
                    "/mnt/petrelfs/share_data/username_a/share/mask3d/mask3d_data/langdata/scanrefer/ScanRefer_filtered.json"
                )
            )
            scanrefer_label = [
                i for i in scanrefer_label if i["scene_id"] == f"scene{now_scene_id}"
            ]
            assert len(box_result) == len(scanrefer_label)
            scanrefer_dict = {}
            for sl in scanrefer_label:
                scanrefer_dict[
                    (f"scene{now_scene_id}", sl["object_id"], sl["ann_id"])
                ] = sl
            ...
            assert len(box_result) == len(collect_nr3d), print(
                f"box_result len: {len(box_result)},nr3d gt len : {len(collect_nr3d)}"
            )
            nr3d_gt_dict = {}
            scene_ids = {}
            tmp_ann_id_count = {}
            ...
            for query in tqdm(collect_nr3d, desc="Initializing ground truths"):
                ...
                nr3d_gt_dict[
                    (scene_id, str(object_id), str(tmp_ann_id_count[scene_obj_key]))
                ] = {
                    "aabb_bound": gt_bbox,
    ...
    v.save(f"visualizations/{pc_path.split('/')[-1].split('.')[0]}")
