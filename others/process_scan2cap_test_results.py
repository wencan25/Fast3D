...
gt_annos = json.load(
    open(
        "/mnt/petrelfs/my_username/share_hw/Chat-3D-v2/annotations/scannet_val_attributes.json"
    )
)
tot_len = 0
...
preds = json.load(
    open(
        "/mnt/petrelfs/my_username/share_hw/Chat-3D-v2/outputs/20240829_220503_lr5e-6_ep3_scanrefer#obj_align#nr3d_caption#scan2cap#scanqa#sqa3d#multi3dref__scan2cap_test__chatscene/preds_epoch-1_step0_scan2cap_test.json"
    )
)
...
id2class = {}
with open(
    "/mnt/petrelfs/my_username/share_hw/Chat-3D-v2/annotations/scannet/scannetv2-labels.combined.tsv",
    "r",
) as f:
...
for pred in tqdm(preds):
    ...
    pred_file = f"/mnt/petrelfs/my_username/share_hw/Chat-3D-v2/tmp_files/Mask3DInst/test/{scene_id}.txt"
    ...
    try:
        sem_prob[id2class[class_id]] = 1.0
    except:
        breakpoint()
